...
bool compatibleInput(CheckData &ckd, CHE *actParm, CHE *formParm, const CContext &callCtx, Category callObjCat)
{
  ...
  if (parm->IsIfStmExpr()) {
    ((IfExpression*)parm)->targetDecl = formTypeDecl;
    ((IfExpression*)parm)->targetCtx = ckd.tempCtx;
    ((IfExpression*)parm)->targetCat = formCat;
    ((IfExpression*)parm)->callObjCat = callObjCat;
    ok &= parm->Check(ckd);
  }
  else {
  ...
  case enumConst_T:
    ...
    if (decl) {
      ...
      && !parentObject->parentObject) {
        ((Reference*)this)->refID = TID(((SelfVar*)parentObject)->myDECL->OwnID,0);
        ((Reference*)this)->refName = ((SelfVar*)parentObject)->myDECL->FullName;
        break;
      }
...
{
  CHE *chp, *chpActIn, *chpFormIn, *branchStm, *precedingBranch;
  SynObject *opd, *opd1;
...
void VarAction::CheckLocalScope (CheckData &ckd, SynObject *synObj)
{
  ...
  for ( ; synObj; synObj = synObj->parentObject) {
    ...
    case run_T:
      tempNo++;
      varName = (VarName*)((Run*)synObj)->varName.ptr;
      elemTypePtr = (Reference*)((Run*)synObj)->objType.ptr;
      if (NoPH(varName)) {
        typeID = elemTypePtr->refID;
        ADJUST4(typeID);
        if (Action(ckd,varName,typeID)) return;
...
//  unsigned nInputs, nOutputs;
  ...
  if (myDECL->DeclType == Function)
    ckd.selfTypeDECL = myDECL->ParentDECL;
  else
    ckd.selfTypeDECL = myDECL;
  ckd.inINCL = ckd.myDECL->inINCL;

  ckd.document->NextContext(myDECL, ckd.lpc);
...
bool BinaryOp::IsOptional (CheckData &ckd) {
  return flags.Contains(isOptionalExpr);
}
...
bool BinaryOp::Check (CheckData &ckd)
{
  ...
  if (declFuncClass->TypeFlags.Contains(isAbstract)) {
    SetError(ckd,&ERR_OperatorUndefined);
    ERROREXIT
  }
  ...
  if (opd2->IsIfStmExpr()) {
    ((IfExpression*)opd2)->targetDecl = formTypeDecl;
    ((IfExpression*)opd2)->targetCtx = ctx;
    ((IfExpression*)opd2)->targetCat = formCat2;
    ((IfExpression*)opd2)->callObjCat = callObjCat;
...
void VarName::ExprGetFVType (CheckData &ckd, LavaDECL *&decl, Category &cat, SynFlags& ctxFlags)
{
  ...
  case caseType_T:
    ...
    break;
  case run_T:
    ((SynObject*)((Run*)parentObject)->objType.ptr)->ExprGetFVType(ckd,decl,cat,ctxFlags);
    break;
  case new_T:
...
Reference *VarName::TypeRef ()
{
  ...
  case caseType_T:
    ...
    break;
  case run_T:
    type = (SynObject*)((Run*)parentObject)->objType.ptr;
    break;
  case new_T:
...
bool Assignment::Check (CheckData &ckd)
{
  ...
  if (((SynObject*)exprValue.ptr)->IsIfStmExpr()) {
    ...
    ((IfExpression*)exprValue.ptr)->targetDecl = targetDecl;
    ((IfExpression*)exprValue.ptr)->targetCtx = targetCtx;
    ((IfExpression*)exprValue.ptr)->targetCat = catTarget;
  ...
  if (!((SynObject*)exprValue.ptr)->IsIfStmExpr()) {
    ...
    if (declSource == (LavaDECL*)-1)
      if (targObj->IsOptional(ckd)) // "nothing" admissible?
        return true;
      else {
...
bool FuncExpression::IsOptional (CheckData &ckd)
{
  return flags.Contains(isOptionalExpr);
}
...
bool FuncExpression::Check (CheckData &ckd)
{
  ...
  if (callExpr) {
    ...
    if (!objTypeDecl) {
      if (!((Expression*)handle.ptr)->flags.Contains(brokenRef))
        ((SynObject*)handle.ptr)->SetError(ckd,&ERR_UndefType);
      if (((SynObject*)function.ptr)->primaryToken == FuncPH_T)
        ((SynObject*)function.ptr)->primaryToken = FuncDisabled_T;
    }
    else if (((SynObject*)function.ptr)->primaryToken == FuncDisabled_T) {
  ...
  case assignFX_T:
    ...
    else if (((LavaDECL*)chpFormOut->data)->TypeFlags.Contains(isOptional))
      flags.INCL(isOptionalExpr);
    else
      flags.EXCL(isOptionalExpr);
...
bool IfExpression::Check (CheckData &ckd)
{
  ...
  for (chp = (CHE*)ifThens.first;
    ...
    if (((SynObject*)opd->thenPart.ptr)->IsIfStmExpr()) {
      ((IfExpression*)opd->thenPart.ptr)->targetDecl = targetDecl;
      ((IfExpression*)opd->thenPart.ptr)->targetCtx = targetCtx;
      ((IfExpression*)opd->thenPart.ptr)->targetCat = targetCat;
      ((IfExpression*)opd->thenPart.ptr)->callObjCat = callObjCat;
    }
    ok &= opd->Check(ckd);
    if (!((SynObject*)opd->thenPart.ptr)->IsIfStmExpr()) {
      ckd.tempCtx = ckd.lpc;
  ...
  if (elsePart.ptr) {
    if (((SynObject*)elsePart.ptr)->IsIfStmExpr()) {
      ((IfExpression*)elsePart.ptr)->targetDecl = targetDecl;
      ((IfExpression*)elsePart.ptr)->targetCtx = targetCtx;
      ((IfExpression*)elsePart.ptr)->targetCat = targetCat;
      ((IfExpression*)elsePart.ptr)->callObjCat = callObjCat;
    }
    ok &= ((SynObject*)elsePart.ptr)->Check(ckd);
    if (!((SynObject*)elsePart.ptr)->IsIfStmExpr()) {
      ckd.tempCtx = ckd.lpc;
...
bool Run::Check (CheckData &ckd)
{
  ...
  if (itf.ptr)
    ok &= ((SynObject*)itf.ptr)->Check(ckd);
//  if (orgunit.ptr)
//    ok &= ((SynObject*)orgunit.ptr)->Check(ckd);
  if (url.ptr)
    ok &= ((SynObject*)url.ptr)->Check(ckd);

  if (!initializerCall.ptr) { // initiator case
    if (IsPH(objType.ptr))
      ERROREXIT

    chpActIn = (CHE*)inputs.first;
    tidType = ((Reference*)objType.ptr)->refID;
    ADJUST4(tidType);
    chpFormIn = GetFirstInput(&ckd.document->IDTable,tidType);
    callCtx = ckd.lpc;
    callObjCat = unknownCategory;
    while (chpFormIn) {
      // locate act. parm. and reposition it if necessary:
...
//    isStateObj = (cat == stateObj ? true : false);
    ...
    chpExec = (CHE*)typeDECL->NestedDecls.last;
    if (!chpExec)
      ((Reference*)objType.ptr)->SetError(ckd,&ERR_EmptyExec);
    execDECL = (LavaDECL*)chpExec->data; // ??
    if (execDECL->DeclType != ExecDef)
      ((Reference*)objType.ptr)->SetError(ckd,&ERR_EmptyExec);
#endif
  }
  else {
    ok &= ((SynObject*)initializerCall.ptr)->Check(ckd);
#ifdef INTERPRETER
    ckd.currentStackLevel++;
    ((ObjReference*)((FuncStatement*)initializerCall.ptr)->handle.ptr)->stackPos = ckd.currentStackLevel;
    typeDECL = ckd.document->IDTable.GetDECL(((Reference*)((FuncStatement*)initializerCall.ptr)->function.ptr)->refID,ckd.inINCL);
    typeDECL = typeDECL->ParentDECL; // ??
    if (ckd.currentStackLevel > ckd.stackFrameSize)
      ckd.stackFrameSize = ckd.currentStackLevel;
