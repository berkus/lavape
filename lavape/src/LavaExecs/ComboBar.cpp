...
void CComboBar::ToggleQNames()
{
	int indT, indST, indO, indE, indComp, indNew, indSNew, indAttac, indCallInt;

	indT = m_TypesCtrl->currentIndex();
	indST = m_SetTypesCtrl->currentIndex();
	indO = m_ObjectsCtrl->currentIndex();
	indE = m_EnumsCtrl->currentIndex();
	indComp = m_CompoObjIntCtrl->currentIndex();
	indNew = m_NewCtrl->currentIndex();
	indSNew = m_SNewCtrl->currentIndex();
	indAttac = m_AttachCtrl->currentIndex();
	indCallInt = m_CallIntCtrl->currentIndex();
    ResetComboItems(m_TypesCtrl);
    ResetComboItems(m_ObjectsCtrl);
    ResetComboItems(m_SetTypesCtrl);
    ResetComboItems(m_EnumsCtrl);
    ...
    CExecFields* execFields = new CExecFields(this, myDECL, ((SelfVar*)ExecDECL->Exec.ptr)->selfCtx);
    delete execFields;
    m_TypesCtrl->setCurrentIndex(indT);
    m_ObjectsCtrl->setCurrentIndex(indST);
    m_SetTypesCtrl->setCurrentIndex(indO);
    m_EnumsCtrl->setCurrentIndex(indE);
    m_CompoObjIntCtrl->setCurrentIndex(indComp);
    m_NewCtrl->setCurrentIndex(indNew);
    m_SNewCtrl->setCurrentIndex(indSNew);
    m_AttachCtrl->setCurrentIndex(indAttac);
    m_CallIntCtrl->setCurrentIndex(indCallInt);
}
...
void CComboBar::OnSelendokComboObjects(int pos)
{
  ...
  if (lastOK) {
    ...
    ((CExecView*)myExecView)->OnInsertObjRef(lastSelStr, data->IDs,false);
  }
  else
...
void CComboBar::OnSelendokComboEnums(int pos)
{
  ...
  if (lastOK) {
    if (lastCombo == objSetEnumCombo)
      ((CExecView*)myExecView)->OnInsertRef(lastSelStr, lastSelTID);
    else {
      EnumMenu.clear();
...
void CComboBar::OnNewPFunc()
{
  ...
  CExecView *view = (CExecView*)myExecView;
  id.nID = myDoc->MakeFunc(SelfTypeDECL, false, view);
  id.nINCL = 0;
  if (id.nID > 0) {
    funcDecl = myDoc->IDTable.GetDECL(id);
	if (ExecDECL->TreeFlags.Contains(qualifiedNames))
      refName = QString(funcDecl->FullName.c);
	else 
      refName = QString(funcDecl->LocalName.c);
...
void CComboBar::OnEnumMenu(QAction *action)
{
  ...
  ((CExecView*)myExecView)->OnInsertEnum(lastSelStr, lastSelTID);
}
...
void CComboBar::OnSelendokComboSubObjects(int pos)
{
  ...
  if (lastOK) {
    ...
    ((CExecView*)myExecView)->OnInsertObjRef(lastSelStr, data->IDs,true);
  }
  else
...
void CComboBar::OnSelendokComboClassFuncs(int pos)
{
  ...
  if (lastOK) {
    ...
    ((CExecView*)myExecView)->OnInsertRef(lastSelStr, idata->funcID, true, &idata->vtypeID);
  }
  else
...
void CComboBar::OnSelendokComboBaseInis(int pos)
{
  ...
  lastOK = pos && (pos > 0);
  if (lastOK)
    ((CExecView*)myExecView)->OnInsertRef(lastSelStr, lastSelTID, true);
  else
...
void CComboBar::AddLocal(const TID& id, const DString& name, const TID& typeID, bool subst)
{
  ...
  accuIDs.Append(che);
  data->IDs = accuIDs;
  addItemAlpha(m_ObjectsCtrl, QString(name.c),QVariant::fromValue(data));//sort#
  ...
  if (typeID.nID >= 0) {
    ...
    if (typeDECL) {
      ...
      if (typeDECL->SecondTFlags.Contains(isSet)) {
        ...
        data->QName = name;
        data->IDs = accuIDs;
        addItemAlpha(m_SetObjectsCtrl, QString(name.c),QVariant::fromValue(data));//sort#
      ...
      if (var) {
        ...
        if ((forHandle || fmvType) && (forCopy || catComp == category || category == anyCat)) {
          if (forHandle || !forHandle &&
            ...
            addItemAlpha(m_CompaObjectsCtrl, m_ObjectsCtrl->itemText(pos),QVariant::fromValue(ndata));//sort#
            showObjs = true;
          }
        }
      }
...
//     itemComp = new CComboBoxItem(item->text(), item->itemData());
      addItemAlpha(m_CompaTypesCtrl, m_TypesCtrl->itemText(pos),m_TypesCtrl->itemData(pos));//sort#
      withTypes = true;
    }
  }
  ...
  for (pos = 1; pos < cnt; pos++) {
    ...
    if (compatibleTypes(ckd, typeDecl, compContext, decl, context)) {
//      itemComp = new CComboBoxItem(item->text(), item->itemData());
      addItemAlpha(m_CompaBTypesCtrl, m_BasicTypesCtrl->itemText(pos),m_BasicTypesCtrl->itemData(pos));//sort#
      withBTypes = true;
    }
  }
  ...
  */
  ...
  for (che = (CHE*)decl->NestedDecls.first; che; che = (CHE*)che->successor) {
    ...
    if ((elDECL->DeclType == Function)
      ...
      addItemAlpha(m_BaseInisCtrl, QString(elDECL->FullName.c),QVariant::fromValue(item));//sort#
    }
  }
...
//            && (elDECL->op == OP_noOp)
		 ...
		 if (sameName || ExecDECL->TreeFlags.Contains(qualifiedNames)) {
            item = new CComboBoxItem(TID(elDECL->OwnID, elDECL->inINCL));
            addItemAlpha(funcBox, QString(elDECL->FullName.c),QVariant::fromValue(item));//sort#
          }
          else {
            item = new CComboBoxItem(TID(elDECL->OwnID, elDECL->inINCL));
            addItemAlpha(funcBox, QString(elDECL->LocalName.c),QVariant::fromValue(item));//sort#
          }
        }
      }
...
//            && (elDECL->op == OP_noOp)
          ...
          addItemAlpha(m_ClassFuncsCtrl, QString(elDECL->FullName.c),QVariant::fromValue(idata)); //sort#
        }
      }
    }
...
//          && (elDECL->op == OP_noOp)
        ...
        if (sameName || ExecDECL->TreeFlags.Contains(qualifiedNames)) {
          ...
          item = new CComboBoxItem(TID(elDECL->OwnID, elDECL->inINCL));
          addItemAlpha(funcBox, QString(elDECL->LocalName.c),QVariant::fromValue(item));
        }
      }
    }
  }
    ...
    for (che = (CHE*)decl->NestedDecls.first; che; che = (CHE*)che->successor) {
      ...
      if ((elDECL->DeclType == Function)
        ...
        && elDECL->TypeFlags.Contains(isStatic)) {
          item = new CComboBoxItem(TID(elDECL->OwnID, elDECL->inINCL));
		  if (ExecDECL->TreeFlags.Contains(qualifiedNames)) 
			addItemAlpha(m_StaticFuncsCtrl, QString(elDECL->FullName.c),QVariant::fromValue(item));
		  else
			addItemAlpha(m_StaticFuncsCtrl, QString(elDECL->LocalName.c),QVariant::fromValue(item));
      }
    }
...
//            && (cheDECL->op == OP_noOp)
          ...
          if (sameName || ExecDECL->TreeFlags.Contains(qualifiedNames)) {
            ...
            addItemAlpha(funcBox, QString(cheDECL->FullName.c),QVariant::fromValue(item));//sort#
          }
          else {
            item = new CComboBoxItem(TID(cheDECL->OwnID, cheDECL->inINCL));
            addItemAlpha(funcBox, QString(cheDECL->LocalName.c),QVariant::fromValue(item));          }
        }//function
        che = (CHE*)che->successor;
      }//while che
    }
...
//              && (ElDECL->op == OP_noOp)
            ...
            if (sameName || ExecDECL->TreeFlags.Contains(qualifiedNames)) {
              item = new CComboBoxItem(TID(ElDECL->OwnID, ElDECL->inINCL));
              addItemAlpha(funcBox, QString(ElDECL->FullName.c),QVariant::fromValue(item));
            }
            else {
              item = new CComboBoxItem(TID(ElDECL->OwnID, ElDECL->inINCL));
              addItemAlpha(funcBox, QString(ElDECL->LocalName.c),QVariant::fromValue(item));
...
//  int pos, pos2, pos3, *pMax, *pMax2, *pMax3=0;
  ...
  if (combo) {
    if (Bar->myDECL && Bar->myDECL->FullName.l && !Bar->ExecDECL->TreeFlags.Contains(qualifiedNames)) {
    ...
    if (combo2) {
      ...
      addItemAlpha(combo2, QString(Label.c),QVariant::fromValue(item));//sort#
    }
    if (combo3) {
      item = new CComboBoxItem(item->itemData());
      addItemAlpha(combo3, QString(Label.c),QVariant::fromValue(item));//sort#
    }
  }
}
...
bool CExecFields::SameField(TDODC& IDs1, TDODC& IDs2, bool& replace)
{
  ...
  while (che1 && che2) {
    if ( ( ((TDOD*)che1->data)->ID.nID != ((TDOD*)che2->data)->ID.nID)
      ...
      if ( Bar->myDoc->IDTable.InheritsFrom(((TDOD*)che1->data)->ID, 0, ((TDOD*)che2->data)->ID, 0)) {
        replace = true;
        return false;
      }
      else
        if (!Bar->myDoc->IDTable.InheritsFrom(((TDOD*)che2->data)->ID, 0, ((TDOD*)che1->data)->ID, 0))
          return false;
...
void CExecFields::AddToBox(LavaDECL** pdecl, DString& name, QComboBox* fieldList)
{
  ...
  if (pos >= 0) {
    ...
    if (!found) {
      if (replace) {
        ...
        if (!fdata->withClass && !Bar->ExecDECL->TreeFlags.Contains(qualifiedNames)) {
  ...
  if (found)
    ...
    addItemAlpha(fieldList, QString(name.c),QVariant::fromValue(data));//sort#
}
