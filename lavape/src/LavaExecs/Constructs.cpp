...
ROContext SynObject::ReadOnlyContext () {
  ...
  //if (!((SelfVar*)obj)->execDECL->ParentDECL->TypeFlags.Contains(isStateObjectY))
  ////&& !((SelfVar*)obj)->execDECL->ParentDECL->TypeFlags.Contains(isAnyCatY))
  //  return roExec;
  //else
    return noROContext;
}
...
#ifdef INTERPRETER
QString SynObject::whatNextText() { return QString::null; }
#else
QString SynObject::whatNextText() {
  switch (primaryToken) {
  case CrtblPH_T:
    break;
  case Exp_T:
  case ExpOpt_T:
    return QApplication::tr("<p>Insert an executable <font color=red><i><b>Lava</b></i></font> expression "
  ...
  case FuncPH_T:
    if (((FuncExpression*)parentObject)->handle.ptr)
      return QApplication::tr("Select a function from the function combo-boxes at the top of the exec window.</p>");
    else
      return QApplication::tr("<p>This is a <b>placeholder for a reference to a static function</b>."
      "<br><br>First select an interface containing the desired static function from the type combo-boxes "
      "at the top of the exec window.<br><br>Then select a static function from the function combo-box "
      "which appears to the right of the type combo-boxes.</p>");
  case FuncDisabled_T:
    return QApplication::tr("<p>This is a <b>disabled placeholder for a function reference</b>."
      "<br><br>Insert a call expression first to enable it.</p>");
  case TDOD_T:
    return QApplication::tr("<p>Select a member variable from the \"Members\" "
      "combo-box at the top of this window (if a member of the required type exists!). "
      "<a href=\"../whatNext/Var_wn.htm\">More...</a></p>");
  case ObjPH_T:
    return QApplication::tr("<p>Select a variable from the \"Variables\" "
  ...
  case SetAttribPH_T:
    break;
  case Stm_T:
    return QApplication::tr("<p>Insert an executable <font color=red><i><b>Lava</b></i></font> statement "
      "by clicking one of the enabled statement buttons on the keyword toolbar "
      "or on the operations toolbar. "
      "<a href=\"../whatNext/Stm_wn.htm\">More...</a></p>");
  case TypePH_T:
  case TypeRef_T:
    return QApplication::tr("<p>Select a type from the \"Types\" or "
      "\"Basic types\" combo-boxes at the top of this window. "
      "<a href=\"../whatNext/Type_wn.htm\">More...</a></p>");
  case VarPH_T:
    return QApplication::tr("<p>Enter a variable name  "
      "or insert another such placeholder; "
      "<a href=\"../whatNext/VarPH_wn.htm\">details...</a></p>");
  case VarName_T:
    return QApplication::tr("<p>Edit this variable name  "
  ...
  default:
    if (((CExecView*)execView)->text->currentSynObj->StatementSelected(((CExecView*)execView)->text->currentSelection))
      ...
      "by clicking one of the enabled statement buttons on the keyword toolbar "
        "or on the operations toolbar. "
        "<a href=\"../whatNext/Statement_wn.htm\">More...</a></p>");
    else if (((CExecView*)execView)->text->currentSynObj->ExpressionSelected(((CExecView*)execView)->text->currentSelection))
      return QApplication::tr("<p>Insert an executable <font color=red><i><b>Lava</b></i></font> expression "
        "by clicking one of the enabled expression buttons on the keyword toolbar "
        "or on the operations toolbar. "
        "<a href=\"../whatNext/Expression_wn.htm\">More...</a></p>");
    else
      return QApplication::tr("<p>No specific \"What next?\" help available here</p>");
  }
  return QApplication::tr("<p>No specific \"What next?\" help available here</p>");
}
#endif
