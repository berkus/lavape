...
IMPLEMENT_DYNAMIC_CLASS(IgnoreStatement,Expression)


void CDPIgnoreStatement (PutGetFlag pgf, ASN1* cid, address varAddr,
                         bool baseCDP)

{
  IgnoreStatement *vp = (IgnoreStatement*)varAddr;
  if (cid->Skip()) return;

  if (!baseCDP) CDPpp.CVTSEQUENCE(pgf,cid);
    CDPExpression(pgf,cid,vp,true);
    vp->igVars.CDP(pgf,cid);
  if (!baseCDP) CDPpp.CVTEOC(pgf,cid);
} // END OF CDPIgnoreStatement
...
IMPLEMENT_DYNAMIC_CLASS(IfdefStatement,Expression)


void CDPIfdefStatement (PutGetFlag pgf, ASN1* cid, address varAddr,
                        bool baseCDP)

{
  IfdefStatement *vp = (IfdefStatement*)varAddr;
  if (cid->Skip()) return;
  ...
  if (!baseCDP) CDPpp.CVTSEQUENCE(pgf,cid);
    ...
    vp->elsePart.CDP(pgf,cid);
  if (!baseCDP) CDPpp.CVTEOC(pgf,cid);
} // END OF CDPIfdefStatement


IMPLEMENT_DYNAMIC_CLASS(CondExpression,Expression)


void CDPCondExpression (PutGetFlag pgf, ASN1* cid, address varAddr,
                        bool baseCDP)

{
  CondExpression *vp = (CondExpression*)varAddr;
  if (cid->Skip()) return;
...
IMPLEMENT_DYNAMIC_CLASS(IfExpression,CondExpression)
...
IMPLEMENT_DYNAMIC_CLASS(ElseExpression,CondExpression)
